...
describe('new otter application', () => {
  ...
  test('should build empty app', async () => {
    const relativeProjectPath = path.relative(workspacePath, untouchedProjectPath);
    ...
    if (untouchedProjectPath) {
      const relativeUntouchedProjectPath = path.relative(workspacePath, projectPath);
  ...
  afterAll(async () => {
    try {
      ...
      // process still running in some cases
    }
  });
});
