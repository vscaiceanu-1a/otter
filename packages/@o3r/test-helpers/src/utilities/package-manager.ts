 ...
 * @param args
 */
export function addDashesForNpmCommand(args?: string[]) {
  if (!args) {
    return [];
  }
 ...
 * @param options
 */
export function packageManagerCreate(command: CommandArguments, options: ExecSyncOptions) {
  const { script, args } = command;
  return execCmd([...PACKAGE_MANAGERS_CMD[getPackageManager()].create, script, ...addDashesForNpmCommand(args)], options);
}
